...
public:
	...
	bool DecodeExif(const unsigned char *pExifBuf,int nLen,ofstream &ofs_log);
...
protected:
	...
	int process_SOFn_NoCheck(const unsigned char * Data, int marker);
	int process_SOFn (const unsigned char * Data, int marker);
	int Get16u(void * Short,int &nValue);
	int Get16m(void * Short,int &nValue);
	int Get32s(void * Long,long &nValue);
	int Get32u(void * Long,unsigned long &nValue);

	int Get16u_NoCheck(void * Short,int &nValue);
	int Get16m_NoCheck(void * Short,int &nValue);
	int Get32s_NoCheck(void * Long,long &nValue);
	int Get32u_NoCheck(void * Long,unsigned long &nValue);


	double ConvertAnyFormat(void * ValuePtr, int Format);
	int ConvertAnyFormat(void *ValuePtr,int Format,double &Value);
	...
	unsigned char *m_pstart_pointer;
	int m_nLength;
